...
## Partie 3 — ILYECE : Interactions + Commentaires
...
- [ ] **3.2** `features/comments/services/comment.service.ts` — liste, ajout, réponse, suppression
...
## Partie 4 — ADELIN : Modération + Administration

> Dépend du `AuthService` + `ModoGuard` (Partie 1).

### Modération

- [ ] **4.1** `features/moderation/resource-moderation/` — liste des ressources en attente de validation, bouton publier/dépublier (`PATCH /api/resources/{id}/publish/`)
- [ ] **4.2** `features/moderation/comment-moderation/` — liste des commentaires signalés, bouton modérer (`PATCH /api/comments/{id}/moderate/`)

### Administration

- [ ] **4.3** `features/admin/user-list/` — liste des citoyens (`GET /api/administration/users/`), activer/désactiver
- [ ] **4.4** `features/admin/category-manager/` — CRUD catégories (`/api/administration/categories/`)
- [ ] **4.5** `features/admin/relation-manager/` — CRUD types de relations (`/api/administration/relations/`)
- [ ] **4.6** `features/admin/stats-dashboard/` — tableau de bord stats (`GET /api/stats/dashboard/`)
